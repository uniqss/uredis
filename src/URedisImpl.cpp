...
URedisImpl::URedisImpl(uv_loop_t* loop, EConnMod connMod, EConnMod dbMod,
...
int URedisImpl::cmd(int64_t id, int64_t opid, redisCbFn* cb, void* privdata, const char* format,
                    va_list ap) {
    ConnCfg* cfg = nullptr;
...
int URedisImpl::cmd(int64_t id, int64_t opid, redisCbFn* cb, void* privdata, const char* cmd,
                    int len) {
    ConnCfg* cfg = nullptr;
    if (!_reconnIfNeeded(id, cfg)) return -1;
